 ...
 * or Raises Not_found. *)
let rec find_nth_field_from str field_sep start n =
  if n = 0 then start else (
    let start' = String.index_from str start field_sep in
    find_nth_field_from str field_sep start' (n-1)
  )
...
let extract_field str field_sep line_start field_num fos =
  ...
  let field_stop = String.index_from str field_start field_sep in
...
let find_line fd field_num field_sep float_of_field target block_size_max max_ofs =
  ...
  t_line, String.sub str line_start (line_stop - line_start)
...
let update_file_info f =
  ...
  if sz <> f.size then (
    f.size <- sz ;
    (try f.last_x <- get_last_x f.fd f.size f.block_size f.separator f.x_field.index f.x_field.to_value
     with Not_found -> ()) ;
    if f.first_x = 0. then
      (try f.first_x <- get_first_x f.fd f.size f.block_size f.separator f.x_field.index f.x_field.to_value
       with Not_found -> ()) ;
  )
...
let read_all fd fn sep fos bs sz n t1 t2 =
  ...
  let rec loop prev t i last_i =
    ...
    let prev',last_i' =
      if fst i != last_i then i :: prev, fst i
      else prev, last_i
    and t' = t +. dt in
    let i' = find_line fd fn sep fos t' bs sz in
