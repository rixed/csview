...
- +--config+, name of the directory to save/read configuration to/from (one per
...
Filters: a way to say when to consider a y value depending on other fields.
Very complex and doable on the CSV directly. Also, would likely disprove the
